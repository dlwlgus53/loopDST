...
class Generate_dataclass:
    def __init__(self, tokenizer, data_path_prefix=None,  raw_data = None, log_path=None, log= None, debugging = False, change_bspn = None):
        ...
        self.change_bspn = change_bspn
        if data_path_prefix:
            train_json_path = data_path_prefix + '/multiwoz-fine-processed-tenpercent.json' 
            labeled_init_path = data_path_prefix + '/labeled_init.json' 
            ...
            with open(labeled_init_path) as f:
                labeled_init = json.load(f)
                
        if raw_data:
            self.train_raw_data = raw_data
            
        filtered_data = self.filter_data(self.train_raw_data, labeled_init, True)
        self.value_dict = self.make_value_dict(filtered_data)


    def filter_data(self, raw, filter):
        new_data = []
        for dial in raw:
    ...
    def extract_now_bspn(self,prev_bspn_dict, bspn_dict):
        this_bspn = {}
        for domain in bspn_dict:
            ...
            for slot in bspn_dict[domain]:
                if slot not in prev_bspn_dict[domain]:
                    if domain not in this_bspn:
                        this_bspn[domain] = {}
                    this_bspn[domain][slot] = bspn_dict[domain][slot]
                elif prev_bspn_dict[domain][slot] != bspn_dict[domain][slot]:
                    if domain not in this_bspn:
                        this_bspn[domain] = {}
                    this_bspn[domain][slot] = bspn_dict[domain][slot]
        return  this_bspn
    ...
    def flatten_data(self, data):
        ...
        for session in data: # session is dial
            ...
            for turn_id in range(turn_num):
                ...
                # Belief 있으면 append 없으면 append안함 TODO
            ...
            for turn_id in range(turn_num):
                ...
                if len(curr_turn['this_bspn'].replace("<sos_b> ", "").replace("<eos_b>",""))!=0:
                    for n in range(aug_num):
                        if curr_turn['turn_num'] ==0:
                            bspn, this_bspn = curr_turn['bspn'], curr_turn['this_bspn']
                        else:
