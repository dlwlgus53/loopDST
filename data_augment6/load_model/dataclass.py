...
class DSTMultiWozData:
    def __init__(self, model_name, tokenizer, data_path_prefix,  shuffle_mode='shuffle_session_level', init_label_path = None, save_label_path = None,
        ...
        if data_mode == 'train':
            train_json_path = data_path_prefix + '/example.json'
            ...
            if self.train_data_ratio < 1.0: # TODO save original list
                if init_label_path:
                    print("use save init")
                    data = []
                    with open(f'{init_label_path}/labeled_init.json') as json_file:
                        init_data = json.load(json_file)
                    for dial in train_raw_data:
                        dial_turn_idx = '[d]'+dial[0]['dial_id'] + '[t]0'
                        if dial_turn_idx in init_data:
                            data.append(dial)
                    train_raw_data = data
                else:
        ...
        dev_json_path = data_path_prefix + '/example.json'
        ...
        test_json_path = data_path_prefix +  '/example.json'
