...
def get_overlap_position(input_ids, label_ids, model):
...
def tokenize(input_text,label, tokenizer, change_rate, model):
    ...
    #####
    input_emb = tokenized_input.inputs_embeds.detach().clone()
    overlap_position = get_overlap_position(input_ids, label_ids, model) # GradAug 기반 position 찾기
    start_token = 0
    end_token =2
    mask_idx = 50264
...
def get_will_change_item(raw_data, tokenizer, change_rate, topn, log, model):
    ...
    for dial_idx, dial in enumerate(raw_data):
        for turn in dial:
            for n in range(topn):
                ...
                text = turn['user']
                label = turn['bspn']
                tokenize_mask_text = tokenize(text, label,tokenizer, change_rate, model)
...
if __name__ == '__main__':
    ...
    tokenizer, model = load_model.main.load_T5()
    ...
    dial_turn_id_list, tokenized_masked_list = get_will_change_item(raw_data, tokenizer, args.change_rate, args.topn, log, model)
